...
another database has to replace them. Concretely, ten of the eleven models
import from `sqlalchemy.dialects.postgresql`, and two correctness guarantees
are Postgres semantics rather than SQL:
...
Two other tables — `workspaces` and `audit_logs` — store that ID as a
...
1. **The identity ID is in your data and in already-sent email.**
   ...
   verifying. Change it in the database and two tables of non-FK strings
...
migration for the identity column in three tables; the erasure worker; the
...
Around the model: seven distinct webhook handlers rather than one upsert
([why](/production/subscription-lifecycle)), an HMAC-signed checkout
passthrough ([why](/production/checkout-integrity)), a three-strategy
fallback for resolving which user a webhook belongs to, a price-ID
resolution module, tier configuration, and two scheduled workers.
...
layer, checkout signing, tier and price configuration, two workers, the
...
ownership filter lives in exactly one place, in SQL. Five repositories ship
with the factory; the scaffold generates one per new resource.
...
Nine reserved directories where your code lives, so an upstream release
merge has nothing of yours to collide with. This one is not so much
replaceable as *the thing you would be giving up*: abandon the convention
and every update becomes a manual reconciliation.

Its guarantee is also narrower than the convenient phrasing, and the
[extension boundary page](/extending/domain-boundary) states exactly where —
worth reading before you rely on it, alongside
[Taking an update](/extending/taking-an-update).

### FastAPI + async SQLAlchemy 2.0

Eighty-seven backend modules, async throughout, with `Annotated` dependency
injection as the auth contract: `CurrentUserDep`, `AdminDep`, `DBDep`.
Every router, every repository and every test assumes it.
...
Eight task modules and an eight-entry beat schedule.
...
Ninety-one frontend modules: App Router, server components by default
(roughly two thirds of them), `next-intl` locale-prefixed routing across
eleven locales, route groups, and middleware.
...
- **Replacing Next.js while keeping the shipped UI** is a rewrite of those
  ninety-one modules. The auth gate, i18n routing and payment-script loading
...
read it. Delete them, rewrite them, or ignore them — nothing in the product
depends on their contents.

This row is the honest counterweight to the rest of the page. It is
genuinely cheap to change, and saying so is what should make the "invasive"
verdicts above worth believing. All three are published in full:
